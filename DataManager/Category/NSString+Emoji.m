...
{
    NSDictionary *reversedMap = @{@":+1:": @"\U0001F44D",
                                  @":-1:": @"\U0001F44E",
                                  @":100:": @"\U0001F4AF",
                                  @":1234:": @"\U0001F522",
                                  @":8ball:": @"\U0001F3B1",
    ...
    NSMutableDictionary *forwardMap = [NSMutableDictionary dictionaryWithCapacity:[reversedMap count]];
    [reversedMap enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
        [forwardMap setObject:key forKey:obj];
    }];
...
- (NSString *)stringByReplacingEmojiCheatCodesWithUnicode
{
    ...
    NSMutableString *newText = [NSMutableString stringWithString:self];
...
//    
//    NSString *resultText = self;
//    // Loop through matches in reverse order so the ranges won't be affected
//    for (NSTextCheckingResult *match in [matches reverseObjectEnumerator]) {
//        NSString *code = [self substringWithRange:match.range];
//        NSString *unicode = s_cheatCodesToUnicode[code];
//        
//        if (unicode) {
...
{
    __block  NSString *lastString = [NSString stringWithString:self];
     ...
     ^(NSString *substring, NSRange substringRange, NSRange enclosingRange, BOOL *stop) {
         ...
         if (0xd800 <= hs && hs <= 0xdbff) {
             if (substring.length > 1) {
                 ...
                 if (0x1d000 <= uc && uc <= 0x1f77f) isContain = YES;
             }
         } else if (substring.length > 1) {
             const unichar ls = [substring characterAtIndex:1];
             if (ls == 0x20e3) isContain = YES;
         } else {
             // non surrogate
             if (0x2100 <= hs && hs <= 0x27ff) isContain = YES;
             else if (0x2B05 <= hs && hs <= 0x2b07) isContain = YES;
             else if (0x2934 <= hs && hs <= 0x2935) isContain = YES;
             else if (0x3297 <= hs && hs <= 0x3299) isContain = YES;
             else if (hs == 0xa9 || hs == 0xae || hs == 0x303d || hs == 0x3030 || hs == 0x2b55 || hs == 0x2b1c || hs == 0x2b1b || hs == 0x2b50) isContain = YES;
         }
         
         if(isContain) {
             lastString = [lastString stringByReplacingOccurrencesOfString:substring withString:@""];
         }
     }];
