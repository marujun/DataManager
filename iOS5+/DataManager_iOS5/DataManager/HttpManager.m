...
- (id)init{
    self = [super init];
	if (self) {
        httpClient = [[AFHTTPClient alloc] initWithBaseURL:[NSURL URLWithString:@"http://www.apple.com"]];
...
- (NSMutableURLRequest *)requestWithUrl:(NSString *)url method:(NSString *)method useCache:(BOOL)useCache params:(NSDictionary *)params
{
    ...
    NSMutableURLRequest *request = [httpClient requestWithMethod:method path:url parameters:params];
              ...
              params:(NSDictionary *)params complete:(void (^)(BOOL successed, NSDictionary *result))complete
{
    ...
    if (useCache) {
        ...
        operation = [httpClient HTTPRequestOperationWithRequest:request success:requestSuccessBlock failure:requestFailureBlock];
    }
                               ...
                               complete:(void (^)(BOOL successed, NSDictionary *result))complete
{
    ...
    NSMutableURLRequest *request = [httpClient multipartFormRequestWithMethod:@"POST" path:url parameters:params constructingBodyWithBlock:^(id<AFMultipartFormData> formData) {
    ...
    [operation setDownloadProgressBlock:^(NSUInteger bytesRead, long long totalBytesRead, long long totalBytesExpectedToRead)
     {
         ...
         FLOG(@"download process: %.0f%% (%d/%d)",100*progress,(NSUInteger)totalBytesRead,(NSUInteger)totalBytesExpectedToRead);
         if (process) {
